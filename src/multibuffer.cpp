...
{
    std::cout << "Executing MultiRingBuffer set_size\n";
    ...
    {
        std::cout << "Creating ringbuff " << +i << '\n';
        
        this->buffers.emplace_back(RingBuffer<T, I>(bufferSize, ringSize));

        std::cout << "\tRingbuff " << +i << " created" << '\n';
    }

    std::cout << "\tMultiRingBuffer set_size done\n";
}
...
template <typename T, typename I>
int_fast32_t MultiRingBuffer<T, I>::num_buffers() const
{
...
{
    RingBuffer<T, I>& b = this->buffers.at(0);
    int_fast32_t
